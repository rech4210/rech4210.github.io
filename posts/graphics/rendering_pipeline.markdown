...
```  
![stencil](https://learnopengl.com/img/advanced/stencil_buffer.png)  
stencil buffer는 0과 1로 구성되는데 stencil의 값이 1인 fragment 영역만을 렌더링하고 나머지는 폐기시킨다.  


-----------------------------------------------------
## alpha blending  
![alpha blending](https://i.stack.imgur.com/EFnPb.png)  
> #### 알파 블렌딩이란?
...
* 알파소팅(알파블렌딩)  
  ...
  - 반투명한 오브젝트의 거리가 먼 순으로 먼저 그려서 해결한다. 즉 깊이버퍼 내림차순으로 그려서 해결한다.  
  * 문제점 : 피봇으로 앞뒤를 판정하기 때문에, 완벽하게 앞뒤를 구분하는 것은 사실상 불가능하다.  
* 알파테스팅
  - 알파의 기준값을 가지고 깊이버퍼에 저장할지 안할지 결정한다. 저장을 안하게되면 1이 되므로 뒤에 있는 객체가 갱신되어 보이게 된다.  
  * 문제점 : 기준 값으로 부터 너무 작을시 외각 처리가 부자연스럽고, 반대로 크면 비어보이는 현상이 발생할 수 있다.  
